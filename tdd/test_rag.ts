...
async function run() {
  ...
  try {
    ...
    const catalogA = await dbA.policyCatalogItem.create({
      data: {
        tenantId: tenantA.id,
    ...
    const catalogB = await dbB.policyCatalogItem.create({
      data: {
        tenantId: tenantB.id,
    ...
    await dbA.policyCatalogItem.update({
      where: { id: catalogA.id },
      data: { active: false },
    });

    const verifyCatalog = await dbA.policyCatalogItem.findUnique({
      where: { id: catalogA.id },
    });
    ...
    await dbA.policyCatalogItem.deleteMany();
    await dbB.policyCatalogItem.deleteMany();
