...
export async function GET(req: Request) {
  try {
    ...
    // Query database for actual RAG chunks
    ...
    if (dbChunks && dbChunks.length > 0) {
      chunks = dbChunks.map((c, idx) => ({
        id: c.id,
        chunkIndex: idx + 1,
        tokens: Math.floor(c.chunkText.length / 4),
        similarityScore: Math.max(0.7, 0.99 - idx * 0.04),
        text: c.chunkText,
        vectorId: c.pineconeVectorId,
      }));
    } else {
      chunks = [
        {
