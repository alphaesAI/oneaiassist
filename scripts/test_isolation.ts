...
async function run() {
  ...
  for (const tenant of oldTenants) {
    ...
    // Delete contacts using the tenant-scoped client
    await dbTenant.contact.deleteMany({
  ...
  const contactsForA = await dbA.contact.findMany();
  ...
  const contactsForB = await dbB.contact.findMany();
