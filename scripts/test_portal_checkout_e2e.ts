...
async function main() {
  ...
  const submitRes = await fetch(`http://localhost:3000/api/tenant/${tenant.slug}/checkout/submit`, {
    method: 'POST',
  ...
  const reqOtpRes = await fetch(`http://localhost:3000/api/tenant/${tenant.slug}/portal/auth/request`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
  ...
  const verifyRes = await fetch(`http://localhost:3000/api/tenant/${tenant.slug}/portal/auth/verify`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
  ...
  const exportRes = await fetch(`http://localhost:3000/api/tenant/${tenant.slug}/portal/account/export`, {
  ...
  const deleteRes = await fetch(`http://localhost:3000/api/tenant/${tenant.slug}/portal/account/delete`, {
    method: 'POST',
    headers: { Cookie: cookieHeader },
  });
